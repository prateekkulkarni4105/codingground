...
void leapstep(double x[], double v[], int n, double dt)
...
void accel(double a[], double x[], int n)
...
//int n : number of points
{
    ...
    for (i = 0; i < n; i++)			/* loop over all points...  */
    {
        //for 1a, a[i]=-x[i]; for 1b, a[i]=x[i]; for 1c, a[i]=sin(x[1])
	    a[i] = -sin(x[i]);	        /* use linear force law     */
	}
}
...
void printstate(double x[], double v[], int n, double tnow)
{
//double x[];					/* positions of all points  */
//double v[];					/* velocities of all points */
//int n;						/* number of points         */
